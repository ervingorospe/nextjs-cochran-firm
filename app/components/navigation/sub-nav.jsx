...
export function SubNav({ item, subNav, navigation }) {
  return (
    <Popover
      ...
      {({ open }) => (
        <>
          <Popover.Button className="cursor-pointer" styles={{ border: 'none !important;' }}>
            <span className="flex items-center text-base font-normal uppercase tracking-wider text-link hover:text-link-hover">
              <span>{item.name}</span>
              <svg
          ...
          >
            ...
            >
              {({ close }) => (
                <div className="relative rounded-lg bg-white shadow-lg ring-1 ring-gray-900 ring-opacity-15 overflow-hidden">
                  <div className="relative grid overflow-auto max-h-[80vh]">
                    {
                      subNav?.map(subItem => {
                        if (_.get(subItem, 'fields.showInNavigation')) {
                          ...
                          if (subItems.length > 0) {
                            return <SubItems item={subItem} subNav={subItems} navigation={navigation} key={subItem.name} closeMainPopover={() => close()}/>
                          }
...
const SubItems = ({ item, subNav, navigation, closeMainPopover }) => {
  return (
    ...
    >
      {({ open }) => (
        <>
          <Popover.Button className="cursor-pointer group/child-nav flex items-center border-b border-gray-200 border-opacity-50 px-5 pt-4 pb-3 hover:bg-gray-100 w-full" styles={{ border: 'none !important;' }}>
